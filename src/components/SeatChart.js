...
const SeatChart = ({ occasion, tokenMaster, provider, setToggle }) => {
  ...
  const getSeatsTaken = async () => {
    // we call getSeatsTaken for check what seats are already taken
  ...
  const buyHandler = async (_seat) => {
    ...
    // get the signer its like "msg.sender"
    const signer = await provider.getSigner()
    console.log(await provider.getNetwork())
    // connect contract with a signer and call a mint(pay occasion function) and set his values
